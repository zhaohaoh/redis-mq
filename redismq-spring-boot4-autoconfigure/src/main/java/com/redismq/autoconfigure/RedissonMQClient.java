...
public class RedissonMQClient {
    ...
    public static RedissonClient redisMQRedisson(RedisProperties redisProperties) {
        ...
        if (redisProperties.getSentinel() != null) {
            ...
            if (redisProperties.getUsername() != null && StringUtils.hasText(redisProperties.getUsername())) {
            ...
            if (clusterMethod != null && ReflectionUtils.invokeMethod(clusterMethod, redisProperties) != null) {
                ...
                List<String> nodesObject = (List<String>) ReflectionUtils.invokeMethod(method, clusterObject);
                String[] nodes = convert(nodesObject);
                config = new Config();
                ClusterServersConfig c = config.useClusterServers().addNodeAddress(nodes)
                        .setClientName(clientName);
                if (connectTimeout != null) {
                    c.setConnectTimeout(connectTimeout);
                }
