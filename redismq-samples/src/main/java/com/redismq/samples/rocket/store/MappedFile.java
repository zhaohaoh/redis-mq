...
//    protected TransientStorePool transientStorePool = null;
    ...
    public AppendMessageResult appendMessagesInner(final Message messageExt, final DefaultAppendMessageCallback cb) {
    ...
    /**
     * 预热映射文件  把文件内容提前加载到内核缓存，同时锁定内存地址空间
     */
    public void warmMappedFile(FlushDiskType type, int pages) {
        ...
        for (int i = 0, j = 0; i < this.fileSize; i += MappedFile.OS_PAGE_SIZE, j++) {
            ...
            if (type == FlushDiskType.SYNC_FLUSH) {
                //如果写入mappedByteBuffer的数据超过了指定页数，就做一次强制刷盘
            ...
            // 这段代码是为了能够gc  如果是int的循环是不会进入安全点的。但是睡眠后可以。GuaranteedSafepointInterval=1000这个参数决定了停顿超过这个实际的主线程会停止等待可以执行安全点的gc清理
            if (j % 1000 == 0) {
